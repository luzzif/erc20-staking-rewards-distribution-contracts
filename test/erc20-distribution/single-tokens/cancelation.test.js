...
contract("ERC20Distribution - Single reward/stakable token - Cancelation", () => {
    ...
    it("should succeed in the right conditions", async () => {
        ...
        expect(await erc20DistributionInstance.getRewardTokens()).to.have.length(0);
        expect(await erc20DistributionInstance.getStakableTokens()).to.have.length(0);
        ...
        expect(await erc20DistributionInstance.startingTimestamp()).to.be.equalBn(
            ZERO_BN
        );
        expect(await erc20DistributionInstance.endingTimestamp()).to.be.equalBn(
            ZERO_BN
        );
        expect(
